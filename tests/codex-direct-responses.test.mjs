...
test("direct Codex Responses transport surfaces tool calls when no executor is provided", async () => {
  const { streamCodexDirectResponses } = await loadModule();
  const body = {
    fetch: async () => sse([
      { type: "response.output_item.done", item: { type: "function_call", id: "call-item", call_id: "call-9", name: "send_message", arguments: "{\"text\":\"hello\"}" } },
      { type: "response.completed", response: { id: "resp-surface", output: [], usage: { input_tokens: 30, output_tokens: 6, input_tokens_details: { cached_tokens: 1 } } } }
    ]),
    endpoint: "https://example.invalid/responses",
    model: "gpt-test",
    instructions: "Greet",
  ...
  await assert.rejects(async () => {
    for await (const _event of streamCodexDirectResponses(body)) {}
  }, /did not provide an executor/);
  const surfaced = [];
  for await (const event of streamCodexDirectResponses({ ...body, surfaceToolCalls: true })) surfaced.push(event);
  assert.deepEqual(surfaced, [
    { type: "tool-call", toolCallId: "call-9", toolName: "send_message", args: "{\"text\":\"hello\"}" },
    { type: "done", text: "", responseId: "resp-surface", usage: { inputTokens: 30, outputTokens: 6, cacheReadTokens: 1, cacheWriteTokens: 0 } }
  ]);
});
