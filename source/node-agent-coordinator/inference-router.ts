...
export function parseInferenceRouterTranscriptStore(value: unknown): Store {
  ...
  for (const [agentId, rawEntries] of Object.entries(root.agents as Record<string, unknown>)) {
    ...
    for (const raw of rawEntries) {
      ...
      if (row == null || !["codex", "openrouter"].includes(String(row.provider)) || !["user", "assistant"].includes(String(row.role)) || typeof row.content !== "string" || typeof row.id !== "string" || typeof row.timestampMs !== "number" || (row.clientNonce !== undefined && typeof row.clientNonce !== "string") || (row.richText !== undefined && typeof row.richText !== "string")) continue;
...
export function createCoordinatorInferenceRouter(options: {
  ...
  const execute = async (provider: Exclude<SandInferenceProvider, "cursor">, args: Record<string, unknown>) => {
    ...
    const directTools = await options.dispatchRemote("listRoutedMcpTools", {});
  ...
  return {
    ...
    async dispatch(method: string, args: unknown): Promise<{ handled: boolean; value?: unknown }> {
      ...
      if (["getAgentTranscriptTail", "openAgentTail", "getAgentTranscriptWindow"].includes(method)) {
        ...
        // Remote (host) and local (router) transcripts mint turn ids from
        // independent counters, so one provider switch can leave the same id
        // on both sides. The virtualized renderer keys rows by entry id and
        // throws on duplicates, so collapse conflicts here (later wins).
        const concatenated = [...result.entries, ...(local.agents[agentId] ?? []).map(projectInferenceRouterTranscriptEntry)];
        const seenIds = new Set<string>();
        const dedupedReversed: unknown[] = [];
        for (let index = concatenated.length - 1; index >= 0; index--) {
          const candidate = concatenated[index]!;
          ...
          dedupedReversed.push(candidate);
        }
        const entries = dedupedReversed.reverse();
      ...
      // Codex and OpenRouter keep the host agent loop so turns carry the full tool chain; the Cursor sand-access gate never applies because inference stays on a local backend.
      if (method !== "sendPrompt" || provider === "codex" || provider === "openrouter") return { handled: false };
      const record = asRecord(args) ?? {};
      const agentId = typeof record.agentId === "string" ? record.agentId : "";
