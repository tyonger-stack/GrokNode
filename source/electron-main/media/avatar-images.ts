...
export interface AvatarImageDialogOptions { readonly title: string; readonly properties: readonly ["openFile"]; readonly filters: readonly [{ readonly name: string; readonly extensions: readonly ["png", "jpg", "jpeg", "webp", "gif", "bmp"] }]; }
export interface AvatarImageDeps { getMainWindow(): { isDestroyed(): boolean; webContents: { executeJavaScript(script: string, userGesture: boolean): Promise<unknown> } } | null; createHiddenWindow(options: { readonly show: false }): unknown; showOpenDialog(window: unknown, options: AvatarImageDialogOptions): Promise<{ canceled: boolean; filePaths: readonly string[] }>; createFromPath(path: string): NativeImagePort; createFromBuffer(bytes: Buffer): NativeImagePort; generate(description: string): Promise<{ imageData: string; mimeType: string }>; onEdgeFailure(failure: { leg: string; errorClass: string }): void; language?: DesktopLanguageSource }
...
export interface ImageContextMenuElectronPorts {
  ...
  readonly dialog: {
    showSaveDialog(window: unknown, options: { readonly defaultPath: string; readonly filters: readonly [{ readonly name: string; readonly extensions: readonly ["png", "jpg", "jpeg", "gif", "webp", "svg", "bmp", "avif"] }] }): Promise<{ readonly canceled: boolean; readonly filePath?: string }>;
  };
  readonly Menu: new() => {
...
export function registerImageContextMenu(deps: ImageContextMenuElectronPorts): void {
  ...
  const saveImageFromContextMenu = async (contents: ImageContextMenuContents, params: ImageContextMenuParams): Promise<void> => {
    const saveCopy = desktopMessages(resolveDesktopLocale(deps.language)).avatar;
    try {
      ...
      const result = await deps.dialog.showSaveDialog(owner, {
        ...
        filters: [{ name: saveCopy.filterName, extensions: ["png", "jpg", "jpeg", "gif", "webp", "svg", "bmp", "avif"] }],
      });
  ...
  const buildImageContextMenu = (contents: ImageContextMenuContents, params: ImageContextMenuParams): { readonly items: readonly unknown[]; append(item: unknown): void; popup(options: { readonly window?: unknown }): void } => {
    const localeCopy = desktopMessages(resolveDesktopLocale(deps.language));
    const menuCopy = localeCopy.contextMenu;
    const avatarCopy = localeCopy.avatar;
    ...
    if (hasLink) {
      sections.push([
        new deps.MenuItem({ label: menuCopy.openLink, click: () => void deps.openExternalUrl(linkUrl) }),
        new deps.MenuItem({ label: menuCopy.copyLinkAddress, click: () => deps.clipboard.writeText(linkUrl) }),
      ]);
    }
    if (hasImage) {
      const imageItems: unknown[] = [
        new deps.MenuItem({ label: menuCopy.copyImage, click: () => copyImageFromContextMenu(contents, params) }),
        new deps.MenuItem({ label: menuCopy.saveImage, click: () => void saveImageFromContextMenu(contents, params) }),
      ];
      if (mediaUrl.length > 0) imageItems.push(new deps.MenuItem({ label: menuCopy.copyImageAddress, click: () => deps.clipboard.writeText(mediaUrl) }));
      sections.push(imageItems);
    }
    if (params.isEditable) {
      sections.push([
        new deps.MenuItem({ label: menuCopy.undo, accelerator: "CmdOrCtrl+Z", enabled: params.editFlags.canUndo === true, click: () => contents.undo() }),
        new deps.MenuItem({ label: menuCopy.redo, accelerator: "Shift+CmdOrCtrl+Z", enabled: params.editFlags.canRedo === true, click: () => contents.redo() }),
        new deps.MenuItem({ label: menuCopy.cut, accelerator: "CmdOrCtrl+X", enabled: params.editFlags.canCut === true, click: () => contents.cut() }),
        new deps.MenuItem({ label: menuCopy.copy, accelerator: "CmdOrCtrl+C", enabled: params.editFlags.canCopy === true, click: () => contents.copy() }),
        new deps.MenuItem({ label: menuCopy.paste, accelerator: "CmdOrCtrl+V", enabled: params.editFlags.canPaste === true, click: () => contents.paste() }),
        new deps.MenuItem({ label: menuCopy.selectAll, accelerator: "CmdOrCtrl+A", enabled: params.editFlags.canSelectAll === true, click: () => contents.selectAll() }),
      ]);
    } else if (params.selectionText.length > 0) {
      sections.push([new deps.MenuItem({ label: menuCopy.copy, accelerator: "CmdOrCtrl+C", click: () => contents.copy() })]);
    }
