...
export type ProductionAccountStatus =
...
export interface ElectronProductionServiceFactories {
  ...
  createTranscriptionManager(context: Omit<ProductionServiceContext, "attachments" | "avatarImages" | "localAccount" | "ensureTranscriptionManager">): () => Promise<unknown>;
...
export function createElectronMainProductionComposition(bindings: ElectronMainProductionBindings): ElectronMainProductionComposition {
  ...
  const botTemplates = createBotTemplateController({ createAgent: (request) => requireValue(coordinatorLegs.legs.createAgent, "createAgent")(request) });
  const deepLinks = new SandDeepLinkController({ dispatch: (parsed) => {
  ...
  const initializeServices = (options?: ElectronMainServicesInitializationOptions): Promise<ElectronMainServices> => initialization ??= (async () => {
    try {
      ...
      const localConnector = createSettingsRoutedHostConnector(requireValue(settings, "settings").settingsStore);
      const pauseControl = createSandClientPauseControl<BoxConnectionInfo, { readonly preserveData: boolean; readonly force?: boolean }, RecreateResult, unknown>(
      ...
      const localConnectorWithPause = connectorEgress.wrap(pauseControl.guard(localConnector));
      boxRecovery = createProductionBoxRecovery({
        connector: localConnectorWithPause,
      ...
      const base: Omit<ProductionServiceContext, "attachments" | "avatarImages" | "localAccount" | "ensureTranscriptionManager"> = {
      ...
      localAccount = bindings.services.createLocalAccount(base);
      ensureTranscriptionManager = bindings.services.createTranscriptionManager(base);
      context = { ...base, attachments, avatarImages, localAccount, ensureTranscriptionManager };
