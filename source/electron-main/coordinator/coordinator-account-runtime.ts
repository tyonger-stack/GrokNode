...
function statusAccountSlot(status: CoordinatorAuthStatus): string | null {
...
export function createCoordinatorAccountRuntime<Status extends CoordinatorAuthStatus>(
  ...
  function enqueueReplacementRecovery(): void {
    chain = chain.then(async () => {
      ...
      const nextSlot = statusAccountSlot(settledStatus);
  ...
  return {
    async start(status) {
      ...
      startWork = (async () => {
        try {
          await applyClaim(statusAccountSlot(status), status, true, 0, (settled) => {
    ...
    observe(status) {
      if (isDisposed()) return;
      const version = ++observedVersion;
      const nextSlot = statusAccountSlot(status);
