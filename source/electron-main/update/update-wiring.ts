...
export function createUpdateServiceWiring(deps: {
  ...
  readonly platform?: NodeJS.Platform; readonly env?: NodeJS.ProcessEnv; readonly log?: (message: string) => void; readonly createService?: (options: SandUpdateServiceOptions) => SandUpdateService;
}) {
  ...
  const options: SandUpdateServiceOptions = {
    currentVersion: deps.currentVersion, buildDefaultTrack: deps.buildDefaultTrack, ...(deps.disabledReason === undefined ? {} : { disabledReason: deps.disabledReason }), squirrel: platform === "darwin" ? deps.electron.autoUpdater : null, tempDir: deps.electron.app.getPath("temp"), machineId: deps.machineId, ...(feedBaseUrl === undefined ? {} : { feedBaseUrl }),
