...
export class SandMcpManager {
  ...
  constructor(private readonly options: any) {
    ...
    this.accountDisplayConfigProvider = this.localOnly
      ? async () => this.localDisplayConfig()
    ...
    this.definitionSource = new SandMcpDefinitionSource(
      ...
      this.localOnly
        ? async () => ({ mcpServers: this.settingsStore.getLocalMcpServers?.() ?? {} })
        : options.accountServersProvider == null
    ...
    this.catalog = new SandMcpCatalogFlow({
      ...
      getLocalMcpServers: () => this.settingsStore.getLocalMcpServers?.() ?? {},
      setLocalMcpServers: (servers) => this.settingsStore.setLocalMcpServers?.(servers),
      configureLocalOAuth: (args) => this.backendMcpExec.configureClient(args),
    });
  ...
  private localDisplayConfig(): any {
    const servers = Object.entries(this.settingsStore.getLocalMcpServers?.() ?? {}).sort(([left], [right]) => left.localeCompare(right)).map(([name, config], index) => ({ id: String(index + 1), name, serverIdentifier: name, config, isTeamServer: false, disabledByTeamAdminPolicy: false }));
    return { cacheScope: "local", servers };
  }
  ...
  async addServer(request: any) {
    const name = this.validateInstallableName(request.name),
      config = parseServerConfig(
        request.configJson,
        this.options.parseServerConfig ?? ((value: unknown) => {
          const parsed = parseMcpServerConfig(value);
          if (parsed == null) throw new SandMcpConfigError("Invalid MCP server configuration.");
          return parsed;
        }),
      );
    if (this.localOnly) {
      this.settingsStore.setLocalMcpServers({ ...(this.settingsStore.getLocalMcpServers?.() ?? {}), [name]: config });
    } else await this.addServersToAccount({ [name]: config });
  ...
  async removeServer(rawId: string) {
    ...
    if (this.localOnly) {
      if (row?.serverIdentifier != null) {
        if ("url" in row.config) await this.backendMcpExec.logoutAccount({ serverUrl: row.config.url, accountKey: "default" });
        this.authWatches.clearPendingAuthWatchesForServer(id);
        const servers = { ...(this.settingsStore.getLocalMcpServers?.() ?? {}) };
        delete servers[row.serverIdentifier];
        this.settingsStore.setLocalMcpServers(servers);
      }
      return { state: await this.reloadServers(id), removed: true };
    }
    if (
      row?.pluginId != null &&
      !row.isTeamServer &&
  ...
  listEffectivePlugins() {
    return this.localOnly || this.effectivePluginsProvider == null
      ? Promise.resolve([])
  ...
  refreshAccountConfigInBackground(): void {
    if (this.localOnly) { this.definitionSource.clearCache(); this.boxRuntime?.invalidateToolsCache(); return; }
    this.generation += 1;
  ...
  async resolveDisplayServer(
    ...
    if (options?.requireFreshRead && !this.localOnly && this.accountServersProvider != null) {
