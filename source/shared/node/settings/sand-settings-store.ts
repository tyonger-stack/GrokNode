...
export class SandSettingsStore {
  ...
  private applyPendingMigrations(settings: SandStoredSettings): SandStoredSettings {
    const migrations = new Set(settings.settingsMigrations);
    const nextMigrations = [...settings.settingsMigrations];
    if (!migrations.has(SAND_DOWNGRADE_MAX_FAST_MIGRATION_ID)) nextMigrations.push(SAND_DOWNGRADE_MAX_FAST_MIGRATION_ID);
    if (!migrations.has(SAND_LOCAL_ONLY_ROUTING_MIGRATION_ID)) nextMigrations.push(SAND_LOCAL_ONLY_ROUTING_MIGRATION_ID);
    if (nextMigrations.length === settings.settingsMigrations.length) return settings;
    const migrated = { ...settings, settingsMigrations: nextMigrations, inferenceProvider: settings.inferenceProvider ?? "openrouter", boxRuntime: "local-docker" as const, ...(settings.agentDefaultModel === undefined ? {} : { agentDefaultModel: downgradePersistedFast(settings.agentDefaultModel) }) };
