...
export function createDesktopPreloadBridge(options: {
  ...
  const desktop: Record<string, any> = {
    ...
    account: {
      getStatus: async () => ({ kind: "logged-in", authId: "local", email: "local@localhost", displayName: "Local", isAnysphereUser: false }),
      login: async () => ({ kind: "logged-out", errorMessage: "Cursor cloud login is unavailable in the local-only build." }),
      cancelLogin: async () => ({ kind: "logged-out" }),
      logout: async () => ({ kind: "logged-out" }),
      updateName: async () => ({ kind: "logged-out" }),
      getAvatar: async () => null,
      getWeeklyUsage: async () => null,
      getUsageSummary: async () => null,
      getPrReviewPreferences: async () => null,
      getPrivacyModeEnabled: async () => true,
      getSandAccess: async () => ({ state: "granted", reason: "none" }),
      getSandAccessFresh: async () => ({ state: "granted", reason: "none" }),
      invokeDashboardAction: async () => ({ ok: false, message: "Cursor cloud actions are unavailable in the local-only build." }),
      cancelTrial: async () => ({ ok: false, message: "Cursor trials are unavailable in the local-only build." }),
      onStatusChanged: (_listener: (payload: unknown) => void) => () => {},
    },
    cursorAccount: {
      getStatus: async () => ({ kind: "logged-in", authId: "local", email: "local@localhost", displayName: "Local", isAnysphereUser: false }),
      login: async () => ({ kind: "logged-out", errorMessage: "Cursor cloud login is unavailable in the local-only build." }),
