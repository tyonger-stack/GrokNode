...
import { LocalHttpMcpExec } from "../../../shared/node/mcp/local-http-exec.js";
import { normalizeLocalMcpServers } from "../../../shared/node/settings/sand-settings-store.js";
...
export function createHostMcp(deps: CreateHostMcpOptions): McpHostPort {
  ...
  return {
    ...
    dispose: async () => { await manager.dispose(); await deps.localHttpMcp?.dispose(); }
  };
}
...
export class McpHostService {
  ...
  private readonly localHttpMcp: LocalHttpMcpExec | undefined;
  readonly api;
  constructor(readonly deps: McpHostServiceDeps) {
    ...
    this.localHttpMcp = deps.localOnly ? new LocalHttpMcpExec({
      credentialDirectory: join(getSandRootDir(), "local-mcp-credentials"),
      getServers: () => {
        const settings = deps.settings;
        if (typeof settings !== "object" || settings == null || !("getLocalMcpServers" in settings) || typeof settings.getLocalMcpServers !== "function") return {};
        return normalizeLocalMcpServers(settings.getLocalMcpServers());
      },
    }) : undefined;
    const backendMcpExec = this.localHttpMcp ?? createDashboardSandBackendMcpExec({
    ...
    this.hostMcp = createHostMcp({
      ...(this.localHttpMcp == null ? {} : { localHttpMcp: this.localHttpMcp }),
      localOnly: deps.localOnly === true,
      log: deps.log,
