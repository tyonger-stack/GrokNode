...
import { computeNextRunAt, expandCronAlias, splitScheduleTimeZone } from "../../../shared/automation-schedule.js";
import {
  isGithubCiEventKind,
...
export class SandAutomationCloudSync {
  ...
  shouldScheduleLocally({ agentId, automation }: { agentId: string; automation: { readonly id?: string; readonly trigger: AutomationTrigger } }): boolean {
    if (!isServerSchedulable(automation) || triggerCronSchedules(automation.trigger).length > 0) return true;
...
export interface LocalCronAutomation extends ScheduledCloudAutomation {
...
export class LocalCronScheduler {
  ...
  constructor(private readonly deps: {
    ...
    readonly fire: (agentId: string, automation: LocalCronAutomation) => Promise<unknown>;
  }) {}
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      const ledger = await readLedger(this.deps.ledgerPath);
      let changed = false;
      for (const { agentId, automation } of await this.deps.listAutomations()) {
        if (!automation.isEnabled || automation.id.length === 0) continue;
        const due = triggerCronSchedules(automation.trigger).some(schedule => {
          const next = computeNextRunAt(schedule, automation.lastRunAt ?? automation.createdAt);
          return next != null && next <= now;
        });
        const key = agentId + ":" + automation.id;
        const minute = Math.floor(now / 60_000);
        if (!due || ledger[key] === minute) continue;
        await this.deps.fire(agentId, automation);
        ledger[key] = minute;
        changed = true;
      }
      if (changed) await writeLedger(this.deps.ledgerPath, ledger);
    } finally { this.running = false; }
  }
}

async function readLedger(path: string): Promise<Record<string, number>> {
  try {
...
async function writeLedger(path: string, ledger: Record<string, number>): Promise<void> {
