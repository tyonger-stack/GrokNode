...
function isRoutedProviderReady(provider: string): boolean {
  if (provider === "codex") {
    try {
      return getLocalInferenceCliStatus().codex.authenticated;
    } catch {
      return false;
    }
  }
