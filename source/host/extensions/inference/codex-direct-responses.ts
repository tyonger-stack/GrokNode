...
export type CodexDirectEvent =
  ...
  | { readonly type: "tool-call"; readonly toolCallId: string; readonly toolName: string; readonly args: string }
...
export type CodexDirectOptions = {
  ...
  readonly surfaceToolCalls?: boolean;
...
export async function* streamCodexDirectResponses(options: CodexDirectOptions): AsyncGenerator<CodexDirectEvent> {
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    if (options.executeTool == null) {
      if (options.surfaceToolCalls === true) {
        for (const call of calls) {
          yield { type: "tool-call", toolCallId: call.call_id, toolName: call.name, args: typeof call.arguments === "string" && call.arguments.length > 0 ? call.arguments : "{}" };
        }
        yield { type: "done", text, responseId, usage };
        return;
      }
      throw new Error("Codex requested a tool but Grok Bot did not provide an executor.");
    }
