import { hasLocalDockerShellGrant, type SandAutoReviewInstructions } from "../../../shared/sand-auto-review-instructions.js";
...
import { hasSavedAutoReviewRule } from "./local-auto-review-rules.js";
...
export class AutoReviewService<Classifier = unknown, Auth = unknown> {
  ...
  async resolveApproval(args: { readonly requestId: string; readonly resolution: SandAutoReviewResolution; readonly agentId: string; readonly entryId: string }): Promise<void> {
    if (args.resolution !== "approved" && args.resolution !== "always" && args.resolution !== "denied") throw new Error("Invalid Auto-review resolution.");
    const owner = this.#ownerOf(args.requestId);
    ...
    if (owner?.resolveApproval(args.requestId, args.resolution) != null) return;
