...
import { createLocalSmartModeClassifierExecutor } from "./local-smart-mode-classifier-exec.js";

type AutoReviewAuth = Parameters<typeof createSandBackendSmartModeClassifierExecutor>[0] & { readonly peekAccessToken?: () => string | null };
...
>({
  ...
  start: (context) => {
    ...
    const service = new AutoReviewService({
      ...
      createClassifierExecutor: (credentials) => credentials.peekAccessToken?.() == null ? createLocalSmartModeClassifierExecutor(() => settings.getAutoReviewInstructions()) : createSandBackendSmartModeClassifierExecutor(credentials),
    });
