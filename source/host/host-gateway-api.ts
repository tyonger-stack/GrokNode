...
export function createHostGatewayApi(
  ...
  return {
    ...
    setHostSettings: (args: any) => {
      const localMcpChanged = args.localMcpServers !== undefined && JSON.stringify(method(settings, "getLocalMcpServers")()) !== JSON.stringify(args.localMcpServers);
      const result = method(settings, "setHostSettings")(args);
      if (localMcpChanged) method(deps.extensions.api("mcp").mcp, "refreshAccountConfig")();
    ...
    refreshMcp: async ({ completion, routedAction, routedArgs }: any) => {
      if (routedAction === "local-http") return await method(deps.extensions.api("mcp"), "localHttpControl")(routedArgs);
