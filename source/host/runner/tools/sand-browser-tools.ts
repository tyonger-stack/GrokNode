...
function browserSpecParameters(schema: BrowserToolSchema): z.ZodTypeAny {
  const shape: Record<string, z.ZodTypeAny> = {
    viewId: z.string().optional().describe("Target browser tab ID. If omitted, uses your dedicated tab (created on first use)."),
  };
  for (const [key, values] of Object.entries(schema.enum ?? {})) {
  ...
  for (const key of schema.required ?? []) {
    if (shape[key] === undefined) {
      shape[key] = z.union([z.string(), z.number(), z.boolean(), z.array(z.unknown()), z.record(z.unknown())]);
    }
  }
  return z.object(shape).passthrough();
}

function browserResultProto(output: BrowserDriverOutput): CommunicateUpdateResult {
  if (output.isError === true) return buildErrorResult(output.text);
  const imageKey = output.imageB64 != null && output.imageB64.length > 0 ? stashScreenshot(output.imageB64) : undefined;
  return buildSuccessResult(encodeEnvelope({
    text: output.text,
...
function renderBrowserToolOutput(_ctx: unknown, output: CommunicateUpdateResult) {
  if (output.result.case === "error") return createStringResult(output.result.value.error || "The browser action failed.", true);
  if (output.result.case !== "success") return createStringResult("The browser action completed.");
  const envelope = decodeEnvelope(output.result.value.currentStep);
  if (envelope.imageKey !== undefined) {
    const imageB64 = pendingScreenshots.get(envelope.imageKey);
    pendingScreenshots.delete(envelope.imageKey);
    if (imageB64 !== undefined) return createImageResult(imageB64, "image/png", envelope.text);
  }
  return createStringResult(envelope.text);
}

function defineBrowserTool<Context>(
  driver: SandBrowserDriver<Context>,
  dependencies: BrowserDriverDependencies<Context> & { readonly onPossibleNavigation?: (context: Context) => void },
  spec: BrowserToolSpec,
) {
  const parameters = browserSpecParameters(spec.schema ?? {});
  return createZodAgentTool(spec.id, {
    ...
    execute: withSafeParsedArgs(
      ...
      async (
        ...
        return await interactionHandler.executeToolCall(
          ctx,
          toolCallWrapper({ phase: "executing", tool: spec.name }),
          meta.toolCallId,
          async () => {
            try {
              validateArguments(spec.schema ?? {}, parsedArgs);
              if (dependencies.autoReview !== undefined) {
                const exactAction = toBrowserReviewAction(spec.op, parsedArgs, dependencies.getDefaultViewId());
                await runSandBrowserAutoReviewPreflight({
                  ctx: ctx as unknown as OperationContext,
              ...
              const output = await driver.run(ctx as Context, {
                op: spec.op,
                toolCallId: meta.toolCallId,
                args: parsedArgs,
                ...(spec.skipScreenshot === undefined ? {} : { skipScreenshot: spec.skipScreenshot }),
              });
              if (spec.canNavigate === true && output.isError !== true) {
                dependencies.onPossibleNavigation?.(ctx as Context);
              }
              return browserResultProto(output);
            } catch (error) {
              return browserResultProto({ text: browserErrorMessage(error), isError: true });
            }
          },
          completedToolCall,
        );
    ...
    render: renderBrowserToolOutput,
    serializeError: (error: unknown) => completedToolCall(browserResultProto({ text: browserErrorMessage(error), isError: true })),
  });
}
...
export function createSandBrowserTools<Context>(
  ...
  return BROWSER_TOOL_SPECS.map((spec) => defineBrowserTool(driver, dependencies, spec));
}
