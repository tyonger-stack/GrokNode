...
| 响应超时 | HTTP 408、504、524，或 150 秒没有收到流式响应（与应用自身的首字停滞判定一致，可用 `SAND_FIRST_TOKEN_STALL_DEADLINE_MS` 覆盖） | 观察上游状态和网络质量；状态灯只提示，不会中止原请求 |
...
HTTP 408、504、524 会归类为“响应超时”。连接已经建立但在 150 秒内没有任何流式事件时，也会显示“响应超时”。这个观察计时器读的是应用自己的首字停滞预算，不会中止请求；如果上游随后恢复，完成时状态会更新为正常并隐藏状态灯。

注意区分两种“超时”：模型首字慢（长上下文下 muse-spark 实测约 2-7 秒，属于正常）不会点灯；只有真的静默超过 150 秒才算。`/models` 探测自身另有 10 秒预算，冷缓存下健康端点实测 p99 约 5.1 秒。
