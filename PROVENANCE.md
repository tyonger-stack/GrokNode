...
This repository publishes the macOS arm64 reconstruction only. The pinned installer above is preserved under
`research-archives/original/0.18.0/` through Git LFS, because the public
download URL now answers HTTP 403 and the archived copy is the only way a
fresh clone can obtain the exact build input. Bootstrap verifies both the DMG
and `app.asar` by SHA-256 before use. The Windows x64 installer is out of
scope and is neither listed nor hosted.
