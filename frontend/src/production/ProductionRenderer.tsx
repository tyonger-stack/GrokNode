import { installBotTemplatePreview } from "../extensions/bot-template-preview";
...
export function ProductionRenderer({ bridge, coordinatorPort }: ProductionRendererProps) {
  ...
  useEffect(() => installBotTemplatePreview(bridge), [bridge]);
  ...
  useEffect(() => {
    ...
    void readFreshSandAccess(bridge.account).then((next) => {
  ...
  useEffect(() => {
    ...
    void Promise.all([bridge.account.getStatus(), bridge.onboarding.getSeen(), bridge.getWindowState()]).then(([status, seen, windowState]) => {
    ...
    const stopAccount = bridge.account.onStatusChanged(observeAccount);
      ...
      <div style={{ display: "grid", gridTemplateColumns: `${renderedSidebarLayout.isCollapsed ? SIDEBAR_LAYOUT_BOUNDS.collapsedWidth : renderedSidebarLayout.expandedWidth}px minmax(0, 1fr)`, height: "100%", minHeight: 0, width: "100%" }}>
        <div style={{ display: "grid", gridTemplateRows: "minmax(0, 1fr) auto auto auto", minHeight: 0 }}>
          ...
          <AccountMenu
            ...
            labels={{ about: UI_TEXT.about, changeLimit: "Change limit", included: "Included", ios: "Get Grok Bot for iOS", logOut: UI_TEXT.logOut, onDemand: "On-demand", settings: UI_TEXT.settings, signIn: UI_TEXT.signIn, spendThisCycle: "Spend this cycle", weeklyUsage: "Weekly usage" }}
            onError={setNotice}
