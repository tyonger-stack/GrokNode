...
import { DEFAULT_ROUTER_PROVIDER, loadBackendRouterProvider, loadRouterProvider, saveBackendRouterProvider, saveRouterProvider, type RouterProviderId } from "./router";
...
export function SettingsDesktopSurface({ bridge, coordinatorClient = null, initialSection = "general", isOpen, onClose, onNotice, onStatus, computer }: SettingsDesktopSurfaceProps) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void (async () => {
      const backend = await loadBackendRouterProvider(bridge.agent);
      if (!active) return;
      if (backend != null) {
        setRouterProvider(backend);
        return;
      }
      try {
        const provider = await loadRouterProvider(bridge.agent.clientPersistence);
        ...
        if (section === "router") return (
          <RouterSettingsPanel
            onChange={async (provider) => {
              ...
              try {
                await saveBackendRouterProvider(bridge.agent, provider);
