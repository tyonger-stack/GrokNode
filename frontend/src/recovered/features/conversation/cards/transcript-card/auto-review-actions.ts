...
export type AutoReviewTransportResolution = AutoReviewResolution;
...
export function createAutoReviewApprovalActions(
  ...
  const loadAlwaysAllow = async (): Promise<{ transport: AutoReviewTransportResolution; settled: AutoReviewResolution }> => {
    ...
    if (current == null) throw new Error("Could not load Auto-review settings. Retry or choose Allow once.");
    await dependencies.instructions.setInstructions(appendAutoReviewAllowInstruction(current, proposedRule));
    return { transport: "always", settled: "always" };
  };
  ...
  return {
    ...
    resolve(resolution) {
      ...
      if (disposed || input.agentId == null || (state.status !== "pending" && state.status !== "failed")) return Promise.resolve(state);
