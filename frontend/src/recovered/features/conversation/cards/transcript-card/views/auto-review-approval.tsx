...
function AutoReviewApprovalBody({ approval, entryId, isStale }: { approval: AutoReviewApproval; entryId: string; isStale: boolean }) {
  ...
  const canAct = approval.status === "pending" && (state.status === "pending" || state.status === "failed") && action != null && providers?.scope.agentId != null && !isStale;
  ...
  return <section aria-labelledby={titleId} aria-label="Auto-review approval">
    ...
    {state.status === "failed" ? <p role="alert">{state.error instanceof Error ? state.error.message : String(state.error)}</p> : null}
    {status === "pending" || state.status === "failed" ? <div>
