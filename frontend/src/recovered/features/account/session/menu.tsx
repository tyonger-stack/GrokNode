...
export async function commitAccountName(
  ...
  return next.length === 0 ? null : bridge.account.updateName(next);
}
...
export function AccountMenu({
  ...
  useEffect(() => {
    ...
    void bridge.account.getAvatar().then((value) => {
  ...
  useEffect(() => {
    ...
    void bridge.account.getUsageSummary().then((summary) => {
      if (!active) return;
  ...
  const signIn = async () => {
    ...
    setBusy(true);
    try {
      onStatus(await bridge.account.login());
      onOpenChange(false);
    } catch (reason) {
