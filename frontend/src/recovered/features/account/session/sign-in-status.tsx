...
export interface SignInStatusProps {
  ...
  bridge: Pick<DesktopBridge, "account">;
  cancelLabel: string;
...
export function SignInStatus({
  ...
  if (account.kind === "logging-in") {
    return (
      <div className="sand-onboarding__landing-wait">
        ...
        <span>
          <SandButton onClick={() => void run(() => bridge.account.login())} size="sm" variant="secondary">{reopenLabel}</SandButton>
          <span aria-hidden="true">·</span>
          <SandButton onClick={() => void run(() => bridge.account.cancelLogin())} size="sm" variant="secondary">{cancelLabel}</SandButton>
        </span>
  ...
  return (
    <>
      <SandButton onClick={() => void run(() => bridge.account.login())} size="sm">{signInLabel}</SandButton>
      {error == null ? null : <p aria-live="polite">{error}</p>}
    </>
  );
}
